...
function toIndexableText(markdown) {
  ...
  // into the prose), HTML comments, and MDX/JSX comments. MDX has no <!-- -->
  // syntax (see server.js's injectProvenanceComment), so every release-
  // pipeline draft's provenance comment - including a raw internal GitLab
  // issue URL - used {/* ... */} instead and was never stripped here. Found
  // live: it embedded straight into the vector index, so a customer's
  // chatbot question could genuinely surface that internal URL in a citation.
  t = t.replace(/```[\s\S]*?```/g, ' ');
  t = t.replace(/<!--[\s\S]*?-->/g, ' ');
  t = t.replace(/\{\/\*[\s\S]*?\*\/\}/g, ' ');
