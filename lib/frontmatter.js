...
/** The article's accountable owner - distinct from `last_update.author`,
 *  which is whoever/whatever last EDITED the draft (including the release
 *  pipeline's service account). `owner` only ever changes at PUBLISH time:
 *  it's who most recently reviewed and approved what's live, so it is only
 *  ever set by the /publish handler, never by /save. Read by the release
 *  pipeline to address an update notification to the right person. */
function setFrontmatterOwner(markdown, owner) {
  const fmMatch = /^---\n([\s\S]*?)\n---/.exec(markdown);
  if (!fmMatch) return markdown;
  let fm = fmMatch[1];
  const safeOwner = String(owner).replace(/[\r\n]/g, ' ');
  const inline = `  owner: ${safeOwner}`;
  const ownerRe = /^[ \t]+owner[ \t]*:[^\n]*/m;
  if (ownerRe.test(fm)) {
    fm = fm.replace(ownerRe, inline);
  } else if (/^customProps[ \t]*:/m.test(fm)) {
    fm = fm.replace(/^(customProps[ \t]*:[^\n]*\n)/m, `$1${inline}\n`);
  } else {
    fm = fm.replace(/\s*$/, '') + `\ncustomProps:\n${inline}`;
...
/** Reads the current `customProps.owner`, or null if the article has never
 *  been published (no owner assigned yet). */
function getFrontmatterOwner(markdown) {
  const fmMatch = /^---\n([\s\S]*?)\n---/.exec(markdown);
  if (!fmMatch) return null;
  const m = /^[ \t]+owner[ \t]*:[ \t]*(.+)$/m.exec(fmMatch[1]);
  return m ? m[1].trim() : null;
}

module.exports = {
  setFrontmatterRoles, removeFrontmatterPrivilege, setLastUpdate,
  setFrontmatterOwner, getFrontmatterOwner,
};
