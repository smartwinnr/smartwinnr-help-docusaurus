...
function buildRoughExplanation(change) {
  ...
  // Live-verified failure mode (#9551): the original wording only covered
  // whether a FEATURE is visible at all - it didn't stop the model from
  // naming a role for WHO may perform one action inside an already-visible
  // feature ("If you have Org Admin privileges, you can perform Reindex
  // all"). Both are the same rule from the reader's side: never learn an
  // internal role/permission name either way.
  parts.push('Editorial note: never name a specific role or permission anywhere in the article - neither for whether a feature is visible at all, nor for who is allowed to perform one particular action inside a feature everyone can otherwise see. Do not write things like "Org Admin", "if you have X privileges", or "restricted to admins". These two cases get different treatment: a fully-gated FEATURE still gets a line saying the option may not be visible to everyone, and to contact SmartWinnr support if it is missing - the article is about that feature, so it cannot simply omit it. A single ACTION restricted to a role within an otherwise-visible feature (e.g. an admin-only "Reindex all" button on a page everyone can open) is different: leave that step out of the article entirely - do not describe it, do not hint it exists, do not say it "requires the right permissions". Role-restricted operational actions like this are handled by SmartWinnr support, not documented for customers to attempt themselves. Only include the actions a general user/editor of the feature can actually take.');
