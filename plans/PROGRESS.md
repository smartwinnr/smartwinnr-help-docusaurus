...
| **Ally chatbot - rename + voice + mark** | ✅ done | Renamed "SmartWinnr Help Assistant" → Wynnie → **Ally** (second rename shipped; the slang concern raised against "Wynnie" is what resolved it, over Sira / Asha / Quill). Custom A+sparkle SVG mark in `src/components/ChatBot/AllyMark.tsx`; system prompt at `prompts/ally.md` for conversational voice + threaded follow-ups (last 6 turns sent to OpenAI). |
| **Ally mascot** | ⏸ pending decisions | `branding/ally/designer-brief.md` + PDF generated; brief locked. Name decision now settled (**Ally**); still awaiting brand approver assignment before designer engagement. **Note**: the checked-in PDF still renders the old name - regenerate from the markdown before sending to a designer. |
